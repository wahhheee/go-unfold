...
序章、第一章“Go 语言与运行时”十节、第二章“并发编程”十一节、第三章“网络与操作系统”十节已完成。第一章包含 37 道练习、40 组追问、10 个实验；第二章包含 44 道练习、44 组追问、11 个实验；第三章包含 40 道练习、40 组追问、10 个实验；均附真实 Go 验证并逐节检查后单独提交。其余模块保留为课程规划；详细范围见 [第一章交付记录](docs/GO_CHAPTER.md)、[第二章交付记录](docs/CONCURRENCY_CHAPTER.md) 与 [第三章交付记录](docs/NETWORK_CHAPTER.md)。
...
- 共 32 篇已发布内容、124 道随堂单选题，支持逐项解释、错题筛选、按节回顾与跨页面答题状态。
...
- 网络与系统实验覆盖 DNS、字节流、窗口、连接池、多路复用、TLS、描述符、页映射、epoll 与请求预算。
...
前三章以 Go 1.27.1 / linux / amd64 为实际核验环境，区分语言版本与工具链版本，注明 Swiss Table、Green Tea、泛型方法、新计时器与泄漏画像等变化的适用范围。扩展库固定为 x/sync v0.23.0、x/time v0.16.0。序章核对了 Redis 8.4 的 `DELEX ... IFEQ`。浏览器实验是受限模型，不提供远程任意代码执行；真实 Go 测试在本地示例模块中运行，模型假设在实验面板内列出。
...
examples/go/                 # 31 组真实 Go 示例、基准和诊断复验记录
...
docs/
  ...
  NETWORK_CHAPTER.md         # 第三章逐节交付与整章验收
```
